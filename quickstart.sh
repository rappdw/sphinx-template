...
if [ "$(uname)" == "Darwin" ]; then
  ...
  sphinx-quickstart -l en --extension=sphinx-math-dollar --extension=myst_parser --extension=sphinxcontrib.confluencebuilder --extension=sphinxcontrib.plantuml --extension=sphinx.ext.graphviz --extension=rinoh.frontend.sphinx --extension=sphinxcontrib.drawio --extension=sphinxcontrib.bibtex --sep --master="index" -t .templates -d confluence_keychain_service=$KEYCHAIN_SERVICE -d confluence_parent="$CONFLUENCE_PARENT" docs
